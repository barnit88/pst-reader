...
namespace core.NDB.Blocks
{
    ...
    public class Block
    {
        ...
        /// </param>
        public Block(MemoryMappedFile file, Bref bref, UInt16 blockDataSize)
        {
            ...
            using (MemoryMappedViewAccessor view = file.CreateViewAccessor((long)bref.ib, actualDataBlockSize))
            {
                ...
                if (bref.ExternalOrInternalBid == ExternalOrInternalBid.External)
                {
                    ...
                    this.DataBlock = new DataBlock(blockDataBytes,blockTrailerDataBytes);
                }
                else if (bref.ExternalOrInternalBid == ExternalOrInternalBid.Internal)
                {
                    ...
                    this.btype = view.ReadByte(0);
                    this.cLevel = view.ReadByte(1);
                    // XBLOCK or XXBLOCK
                    if (btype == 0x01)
                    {
                        // XBLOCK
                        if (cLevel == 0x01)
                        {
                            ...
                            this.BlockType = BlockType.XBLOCK;
                            this.XBlock = new XBlock(blockDataBytes, blockTrailerDataBytes);
                        }
                        // XXBLOCK
                        else if (cLevel == 0x02)
                        {
                            this.BlockType = BlockType.XXBLOCK;
                            this.XXBlock = new XXBlock(blockDataBytes, blockTrailerDataBytes);
                        }
                    }
                    // SLBLOCK OR SIBLOCK
                    else if (btype == 0x02)
                    {
                        ...
                        if (cLevel == 0x00)
                        {
                            this.BlockType = BlockType.SLBLOCK;
                            this.SLBlock = new SLBlock(blockDataBytes, blockTrailerDataBytes);
                        }
                        //SIBLOCK
                        if (cLevel == 0x01)
                        {
                            this.BlockType = BlockType.SIBLOCK;
                            this.SIBlock = new SIBlock(blockDataBytes, blockTrailerDataBytes);
                        }
                    }
                }
            }
